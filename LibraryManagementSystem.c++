...
int main()
{
	...
	while(num==1)//Inventory Setup
	{
		...
		getchar();
		std::cout<<"Title: ";
		std::string title; getline(std::cin, title);
		
		std::cout<<"Author: "; 
		std::string author; getline(std::cin, author);
		std::cout<<"Publisher: ";
		std::string publisher; getline(std::cin, publisher);
		std::cin.clear();
		std::cout<<"Price: ";
	...
	while(num)//Querying The System
	{
		...
		else
		{
			...
			else std::cout<<"Required copies are not in stock\n";
			}
